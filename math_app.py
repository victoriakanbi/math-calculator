...
# --- FILE SYSTEM & ROBUST SAVING ---
FILE_NAME = "calc_data_v2.txt"

def save_history_robust(text):
    """
    Saves to file with extreme safety. 
    Catches ALL errors (Exception) so the app NEVER crashes.
    """
    st.session_state.history_cache = text # Priority 1: RAM
    
    try:
        with open(FILE_NAME, "w") as f:
            f.write(text) # Priority 2: Disk
    except Exception:
        # If OneDrive blocks us, we DO NOT CRASH. We just keep going.
        pass

# --- LOAD STRATEGY ---
# Only load from disk once at startup
if not st.session_state.history_cache:
    try:
        if os.path.exists(FILE_NAME):
            with open(FILE_NAME, "r") as f:
                st.session_state.history_cache = f.read()
    except:
        pass

default_text = st.session_state.history_cache
...
# --- HISTORY TAB ---
with tab_log:
    user_input = st.text_area("Raw Input Log", height=300, value=default_text, help="Edit this to modify past commands")
    if user_input != default_text:
        save_history_robust(user_input) # Safe Save (won't crash if it fails)
...
# --- INPUT SYSTEM ---
new_command = st.chat_input("⚡ Type math here (e.g., 'lap y''+y=0', '14.7 psi to kPa')")

# NEW: INTERCEPT 'CLEAR' COMMAND
if new_command:
    if new_command.strip().lower() == "clear":
        save_history_robust("")
        st.rerun()
    else:
        separator = "\n" if default_text else ""
        updated_text = default_text + separator + new_command
        # CRITICAL FIX: Safe save that won't crash the app
        save_history_robust(updated_text) 
        st.rerun()
