...
with st.sidebar:
    ...
    with tab_help:
        ...
        | **Deriv** | Derivative (Formula/Point/Imp) | `deriv x^3` (Formula)<br>`deriv x^2, 2` (Point) |
        """, unsafe_allow_html=True)
        ...
        if uploaded_log and st.button("📥 Load File into Log", use_container_width=True):
            ...
            try:
                if uploaded_log.name.endswith(".docx"):
                    ...
                    try: content = uploaded_log.getvalue().decode("utf-8")
                    except: content = uploaded_log.getvalue().decode("latin-1")
...
calc_memory = {"np": np, "math": np, "pi": np.pi, "e": np.e, 
               ...
               "ln": np.log,        # ln is Base e (Natural)
               "log": smart_np_log, # log is Base 10 (or custom)
               "log10": np.log10,   # Explicit Base 10
               "log2": np.log2,     # Explicit Base 2
...
sym_memory.update({
    ...
    "ln": sp.log,         # ln is Natural (SymPy default)
    "log": smart_sp_log,  # log is Base 10
...
# Footer for print - HIDDEN WHEN PRINTING
