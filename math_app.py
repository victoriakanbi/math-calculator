...
def display_answer(label, exact_val, warning=None):
    ...
    with history_container.container(border=True):
        ...
        with c2:
            ...
            try: latex_str = sp.latex(exact_val).replace("\\log", "\\ln").replace("\\theta", "\\text{step}")
        ...
        if cmd == "calc:":
            ...
            for idx, f in enumerate(funcs):
                ...
                if "=" in clean_f: 
                    ...
                    # Simplified contour for speed
                    feature_x = np.linspace(st.session_state.min_x, st.session_state.max_x, 100)
                    feature_y = np.linspace(st.session_state.min_y, st.session_state.max_y, 100)
                    X, Y = np.meshgrid(feature_x, feature_y)
                    Z = f_imp(X, Y)
                    fig.add_trace(go.Contour(z=Z, x=feature_x, y=feature_y, contours=dict(start=0, end=0, size=2, coloring='lines'), line=dict(width=2, color=color), showscale=False, name=clean_f))
                else:
            ...
            fig.update_layout(height=400, margin=dict(l=20,r=20,t=20,b=20), xaxis_title=graph_var, yaxis_title="y")
            with history_container.container(border=True):
            ...
            if len(parts) == 2: res = res.subs(var, smart_parse(parts[1]))
            display_answer("Derivative", res)
            
        elif cmd == "convert:":
            # Basic conversion logic
            m = re.search(r"([ \(\)\d\.\-\+eE]+)\s*([a-zA-Z\d_]+)\s+to\s+([a-zA-Z\d_]+)", raw_content, re.IGNORECASE)
            if m:
                val, u_from, u_to = float(eval(m.group(1))), m.group(2), m.group(3)
                res, cat = perform_conversion(val, u_from, u_to)
                if res: 
                    with history_container.container(border=True):
                        st.metric(f"Convert {cat}", f"{format_number(res)} {u_to}", f"{val} {u_from}")
                        st.session_state.ans = res

    except Exception as e:
        # Silently fail for clean UI, or log if needed
        pass
